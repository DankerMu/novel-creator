...
一个面向中文长篇小说的 **“AI 写作 IDE（本地 Web 版）”**：
...
**Lite（轻依赖，最快跑起来）**
- 前端 + 后端 + SQLite
- KG 用 SQLite 的图表（edges/nodes）实现最小可用
- 不启用向量库
...
**注入策略（必做）**
- 预算：Lorebook 注入有 token/字符预算（例如总上下文的 10%～20%），超出则按 `priority`、相关度排序截断
