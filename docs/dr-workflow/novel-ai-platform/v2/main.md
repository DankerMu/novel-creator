# 中文长篇小说 AI 写作平台（本地拉起 Web 版）设计与规格 v0.2
...
一个面向中文**中长篇小说**（10 万～百万字）的 **”AI 写作 IDE（本地 Web 版）”**：
...
> 注：之所以强调”IDE”，是因为仅有”生成器”不足以支撑长篇的迭代、返工、改设定、回溯、复用与一致性治理。

### 1.1a 目标用户画像

> [DR-012] 验证：百万字创作是中国网文市场的真实需求，但需精准定位。

| 用户类型 | 典型字数 | 核心需求 | 本平台匹配度 |
|---------|---------|---------|------------|
| **精品长篇作者**（主要目标） | 20 万～100 万字 | 一致性管理、设定结构化、质量控制 | ✅ 高 |
| **IP/剧本创作者** | 10 万～30 万字 | 角色管理、情节连贯、多线叙事 | ✅ 高 |
| **网文日更作者** | 100 万～500 万字 | 速度、不卡文、日更效率 | ⚠️ 中等 |
| **出版长篇作者** | 10 万～50 万字 | 文学质量、结构控制 | ✅ 高 |

差异化价值：**一致性管理 + 结构化创作工作流**，而非单纯的”AI 生成字数”。
...
- 后端 API（FastAPI/Node 任一实现均可；本规格以 FastAPI 举例）：
  ...
  - AI Orchestrator（LLM 路由与生成管理）：
    - 模型路由：支持 OpenAI-compatible 自定义端点
    - 结构化输出：Instructor + Pydantic 校验（非流式端点）
    - 流式输出：FastAPI SSE（正文生成端点）
    - 事件总线：asyncio.Queue（本地单机足够，无需 Redis）
...
**Lite（轻依赖，最快跑起来）**
- 前端 + 后端 + SQLite
- KG 用 SQLite 双表实现（`kg_nodes` + `kg_edges`，与 Neo4j schema 字段对齐）
  - **可做**：实体 CRUD、关系管理、1~2 跳邻居查询、基本冲突检测、JSON 属性存储
  - **受限**：≥3 跳深度遍历性能下降、无原生图算法（最短路径/中心性）
  - 图可视化由前端统一处理（D3.js/Cytoscape.js），不依赖 Neo4j Browser
- 向量检索：v1 可选 sqlite-vec 扩展（零额外部署）
- **GraphService 抽象接口**：Lite 用 `SQLiteGraphAdapter`，Full 用 `Neo4jAdapter`，上层代码不感知存储差异
...
- `kg_proposals`（抽取候选，待审阅）
  - `id`, `chapter_id`, `payload_json`, `confidence`(float 0~1), `status`(pending/auto_approved/auto_pending/user_approved/rejected), `created_at`
- `kg_evidence`（证据引文，独立于 KG 存储层）
  - `id`, `fact_id`, `chapter_id`, `scene_id`, `paragraph_idx`, `quote_text`, `created_at`
- `workflow_runs`
  - `id`, `project_id`, `workflow_key`, `status`, `input_json`, `output_json`, `logs_md`, `created_at`, `updated_at`
- `workflow_node_runs`（工作流节点执行记录）
  - `id`, `run_id`, `node_id`, `status`(pending/running/completed/failed), `started_at`, `completed_at`, `output_json`, `error_msg`
- `prompts`
...
- `approval_status`（`auto_approved` / `user_approved` / `auto_pending` / `rejected`）
- `evidence`（证据引用）：
  - Neo4j 侧：仅存轻量指针 `evidence_chapter_id`(int) + `evidence_scene_id`(str) + `evidence_paragraph_idx`(int)
  - SQLite 侧：`kg_evidence` 表存完整引文（`fact_id`, `chapter_id`, `scene_id`, `paragraph_idx`, `quote_text`, `created_at`）
  - 反向查询（"某段落被哪些事实引用"）走 SQLite 索引，<1ms
- `status`（例如角色 alive/dead；线索 open/resolved）
- `source`（system_extraction / user_edit）

> 为什么必须 `evidence`：没有证据，图谱会因抽取误差逐渐污染，且无法回溯修正。
>
> [DR-004] 结论：小说 KG 规模（<2000 节点、<5000 边）下性能无压力。采用指针+外部存储混合模式，兼顾遍历效率和全文搜索。

### 6.3 向量检索（可选，分阶段引入）

借鉴 SillyTavern 的”向量存储 + 世界信息 + Personas”并行增强上下文的思路。
- 向量库主要用于：召回”相似场景””伏笔回指””过去提过但没结构化入图的描写”。
- 图谱负责”事实/关系”，向量负责”语义/氛围/隐含线索”。

**分阶段集成路线**（[DR-008] 结论）：
- **MVP**：不集成向量库——KG + Lorebook + 关键词已覆盖 80%+ 上下文需求
- **v1**：集成 **sqlite-vec**（SQLite 扩展，零额外部署），按段落切分（~200 字/段），使用本地 BGE-small-zh 模型
- **v2**：如需更高性能，升级到 Qdrant（Docker）

**去重策略**：向量检索结果与 KG evidence 按 `paragraph_idx` 去重后再注入；向量预算独立（总上下文的 5%~8%），作为”补充上下文”层。
...
**注入策略（必做）**（[DR-005] 结论：关键词+别名覆盖 90%+ 触发需求，语义匹配排 v2）

- **扫描范围**：当前场景 + 前一场景的文本（Scan Depth，参考 SillyTavern）
- **激活逻辑**：
  - MVP：关键词 OR 匹配 + aliases 别名扩展
  - v1：增加 AND 逻辑（`and_keywords`）+ 递归激活（条目 A 的 content 触发条目 B）
- **预算**：Lorebook + KG 共享预算上限（总上下文的 15%~25%），超出按 priority 截断
- **截断策略**：
  1. Constant 条目（Force Active）最先占位
  2. 按 `priority` DESC 排序（建议默认值：主角 100，配角 80，地点 60，规则 40，氛围 20）
  3. 同 priority 按关键词匹配精确度排序
  4. 超出预算的条目从 Bottom 截断到 sentence 粒度
  5. 截断后 token 数 < `reserved` 的条目整条移除
- **条目写作指南**：最重要的信息放前几行（因为 Bottom trim）
...
**审阅与入库（置信度分级模式）**（[DR-009] 结论：逐条审阅负担过重，改为自动分级入库）

- 系统先产出 `kg_proposals`，每条事实附 `confidence` 分数（LLM 自评 + 规则校验加权）
- **入库策略**：
  - High (≥0.9)：**自动入库**，标记 `approval_status: auto_approved`（如明确的角色出场、已知关系）
  - Medium (0.6~0.9)：**自动入库但待确认**，标记 `auto_pending`（如推断性关系、模糊事件）
  - Low (<0.6)：**不入库**，进 pending queue 等待人工审阅
- **生成时查询范围**：`user_approved` + `auto_approved` + `auto_pending`（不查询 `rejected`）
- **UI 展示**：待确认事实以虚线/浅色标识，区别于已确认事实
- **章末提示（非阻断）**：toast 通知 “本章抽取了 N 条新事实，M 条待确认”，不阻断写下一章
- **批量审阅模式**：按类型分组（角色/关系/事件）、一键批量确认、快捷键（A=approve, R=reject, →=next）
- **回滚**：支持”撤销本章所有 auto_approved”，回到上一章的 KG 状态
...
### 8.2 Context Pack 分层与预算分配

> [DR-002] 结论：采用 SillyTavern/NovelAI 的分区预算模式——每层设 `max_budget`（上限）和 `reserved`（保底），未用完的预算自动回流给短期上下文。

**默认预算分配表**（以 32K token 上下文为例）：

| 上下文层 | 预算占比 | ~token 数 | 说明 |
|---------|---------|----------|------|
| 系统约束（System） | 5~10% | ≤2048 | 风格卡、POV、禁忌，较稳定 |
| 长期记忆（Long-term） | 10~15% | ≤4096 | Rolling Summary、阶段目标、未解线索 |
| 结构化事实（KG + Lorebook） | 15~25% | ≤6144 | 按触发动态注入，此为预算上限 |
| 短期上下文（Recent Text） | ≥50% | 剩余全部 | 最近场景原文，保证续写连贯 |

**溢出策略**：预算不足时按优先级逐层降级：
1. 低优先级 Lore 条目截断
2. 压缩 Rolling Summary
3. 缩短 Recent Text（最后手段）

**首章模式**：首章无历史数据，加大 System + Bible 占比，Long-term 和 KG 预算回流给 Recent Text。
...
```json
{
  ...
  "properties": {
    ...
    "entities": {
      ...
      "items": {
        "type": "object",
        "required": ["type", "name", "aliases", "confidence", "evidence"],
        "properties": {
          "type": {"type": "string", "enum": ["Character", "Location", "Organization", "Item", "PlotThread"]},
          "name": {"type": "string"},
          "aliases": {"type": "array", "items": {"type": "string"}},
          "confidence": {"type": "number", "minimum": 0, "maximum": 1, "description": "抽取置信度"},
          "evidence": {
            "type": "object",
...
3) **强约束（Streaming Gate）**（**v1 功能**，MVP 不实现）
- 流式输出时分段计数，达到阈值就中断并把”已写字数/剩余字数”回喂继续生成
- 实现要点（[DR-003] 结论）：
  - soft_limit = N×0.9，hard_limit = N×1.1；到达 soft_limit 后在下一个句号（`。？！…」』）`）处中断
  - 续写 prompt 注入：”从断点处自然衔接，不要重复”+ 最后 2~3 段作为短期上下文
  - 最大续写轮数 ≤3，超出走人工介入
  - 状态持久化到 `workflow_runs` 表，支持中断恢复
- 优点：最精确
- 缺点：工程复杂，需要稳健状态机
...
> [DR-006] 结论：采用「规则引擎 + LLM 推理」混合架构，结构化冲突零成本零误报，语义冲突用 LLM。

### 12.1 规则引擎校验（MVP，零成本）

以下 4 类冲突可完全规则化，数据源为 KG 查询：

- **角色状态冲突**：`IF character.status == 'dead' AND character APPEARS_IN chapter_N THEN conflict`
- **时间线冲突**：`IF event_A.time > event_B.time AND event_A CAUSES event_B THEN conflict`（需 KG Event 节点存 `narrative_day` 字段）
- **持有物冲突**：`IF character_A OWNS item AND character_B OWNS item AND item.unique THEN conflict`
- **线索状态冲突**：`IF plot_thread.status == 'resolved' AND plot_thread MENTIONED_AS 'unknown' IN chapter_N THEN conflict`

另外，**n-gram 重复检测**（纯统计，零成本）可在 MVP 中实现。

### 12.2 LLM 语义校验（v1）

以下冲突需要 LLM 推理（预估准确率 75~85%，单章成本 ~$0.02）：

- **设定矛盾**：正文描述与 Bible/Lore 的语义冲突（如 Lore 说”恐高”但正文写”悬崖边毫不畏惧”）
- **视角漂移**：半规则化（检测第三人称标记）+ LLM 确认
- **风格偏离**：句长分布/标点密度统计 + LLM 评估

默认触发时机：`chapter_mark_done` → 自动跑规则引擎；LLM 校验需用户手动触发或配置自动。
...
- 每条问题必须包含：
  ...
  - `confidence`（规则引擎 = 1.0，LLM = 模型自评分 0~1）
  - `source`（`rule` / `llm`）
  - `evidence`（引用段落）
  - `expected`（应符合的设定/事实）
  - `suggest_fix`（改写建议）
- 默认只展示 high + medium，low 级问题折叠
- 用户可标记”非问题”，训练排除规则降低重复误报
...
> [DR-007] 结论：MVP 不需要外部引擎（Airflow/Prefect/Temporal 均过重），JSON 定义 + Python 装饰器注册即可，核心代码 ~500 行。
...
```json
{
  ...
  “nodes”: [
    ...
    {“id”: “extract_kg”, “handler”: “kg_extraction”, “depends_on”: [“summarize”]},
    {“id”: “update_lore”, “handler”: “lore_suggestion”, “depends_on”: [“summarize”]},
...
**执行引擎核心**：
- DAG 解析 + 拓扑排序 → 按层并行执行（`asyncio.gather`）
- handler 通过 `@engine.register(“handler_name”)` 装饰器注册（白名单，无代码注入风险）
- 状态持久化：`workflow_runs` 表 + 新增 `workflow_node_runs` 表（node_id, status, output_json, error_msg）

**必须支持**：
- 触发器（事件驱动）
- 节点级进度上报（前端通过 SSE 订阅 run_id 状态变更）
- 单节点重试 + 断点恢复（从最后完成节点继续）
- 运行日志与产物可回放

**预定义 handler（MVP）**：`chapter_summary`, `kg_extraction`, `lore_suggestion`, `consistency_check`, `export_chapter`

> Python DSL（用户自定义工作流脚本）排 v2，需配合 RestrictedPython 沙箱。
...
> [DR-011] 结论：结构化数据用 Instructor+Pydantic 非流式校验；正文用 SSE 流式+后置校验。推荐依赖：`instructor`, `pydantic`。

**非流式端点**（结构化 JSON，Instructor + Pydantic 自动校验 + 重试）：
- `POST /api/generate/scene-card` → 返回 `SceneCard` Pydantic model
- `POST /api/generate/plan-next` → 返回结构化大纲

**流式端点**（SSE，正文逐段返回 + 后置校验）：
- `POST /api/generate/scene-draft` → SSE 流式输出，最后一个 event 携带校验结果（字数、角色出场检查）
- `POST /api/generate/rewrite` → SSE 流式输出
...
### 16.3 AI 生成交互（流式 vs 非流式）

- **流式端点**（场景正文、润色）：编辑器中逐段显示生成文本，底部显示实时字数；生成结束后弹出校验摘要（字数偏差、角色出场检查）
- **非流式端点**（场景卡、KG 抽取、摘要）：显示加载状态 → 结果一次性渲染到对应面板

### 16.4 KG 批量审阅 UI

- **章末 toast 提示**（非阻断）：”本章抽取了 N 条新事实，M 条待确认” → 点击进入审阅模式
- **审阅面板**：
  - 分类视图：按类型分组（新角色 / 关系变化 / 事件 / 状态更新），非逐条平铺
  - 置信度过滤：滑块筛选 confidence ≥ X 的事实
  - 批量操作：”全部确认” / “按类型确认” / “全部拒绝”
  - 快捷键：`A` = approve, `R` = reject, `→` = next
  - diff 视图：只展示与上一章 KG 的增量变化（新增/修改/删除）
  - evidence 悬停：鼠标悬停事实时，弹出原文高亮定位
...
> [DR-010] 结论：单人全量 7 模块需 10~11 周，建议拆为两阶段降低交付风险。
> 团队假设：1 人全栈 + AI 辅助开发。技术栈：FastAPI(Python) + React/Next.js(TypeScript) + SQLite。

### MVP-α（3~4 周，核心可用："能写出来"）
- 项目/Chapter/Scene CRUD + 场景卡编辑器
- Story Bible（核心字段 CRUD + 锁定）
- AI 场景生成（SSE 流式输出）
- 基础 Context Pack（Bible + 最近文本）
- 章节摘要（自动）
- 导出 Markdown/TXT

### MVP-β（+2~3 周，特色能力："写得一致"）
- Lorebook（关键词触发注入 + 预算截断）
- KG 抽取 + 置信度分级自动入库（Lite: SQLite 双表）
- Context Pack 增强（+ Lorebook + KG 注入）
- 字数控制（中约束：场景预算 + 扩写/压缩）
- 规则引擎一致性校验（4 类结构化检查）

### v1（稳定创作）
- LLM 语义一致性校验（设定矛盾、视角漂移）
- 工作流引擎（JSON DAG + 章完成自动跑流水线）
- 版本/快照/命名 checkpoint
- 字数强约束（Streaming Gate）
- sqlite-vec 向量检索集成
